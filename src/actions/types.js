export const GET_ERRORS = 'GET_ERRORS';
export const SET_CURRENT_USER = 'SET_CURRENT_USER';
export const SET_GUEST_ID = 'SET_GUEST_ID';

export const GET_AIRPORTS_SUCCESS = 'GET_AIRPORTS_SUCCESS';
export const GET_AIRPORTS_PENDING = 'GET_AIRPORTS_PENDING';

export const GET_FLIGHTS_SUCCESS = 'GET_FLIGHTS_SUCCESS';
export const GET_FLIGHTS_PENDING = 'GET_FLIGHTS_PENDING';

export const GET_TICKETS_SUCCESS = 'GET_TICKETS_SUCCESS';
export const GET_TICKETS_PENDING = 'GET_TICKETS_PENDING';

export const GET_BOOKING_SUCCESS = 'GET_BOOKING_SUCCESS';
export const GET_BOOKING_PENDING = 'GET_BOOKING_PENDING';

export const POST_BOOKING_SUCCESS = 'POST_BOOKING_SUCCESS';
export const POST_BOOKING_PENDING = 'POST_BOOKING_PENDING';

export const GET_BOOKINGS_BY_USER_PENDING = 'GET_BOOKINGS_BY_USER_PENDING';
export const GET_BOOKINGS_BY_USER_SUCCESS = 'GET_BOOKINGS_BY_USER_SUCCESS';

export const GET_TICKETS_BY_USER_PENDING = 'GET_TICKETS_BY_USER_PENDING';
export const GET_TICKETS_BY_USER_SUCCESS = 'GET_TICKETS_BY_USER_SUCCESS';

export const DELETE_TICKET_PENDING = 'DELETE_TICKET_PENDING';
export const DELETE_TICKET_SUCCESS = 'DELETE_TICKET_SUCCESS';

export const DELETE_BOOKING_PENDING = 'DELETE_BOOKING_PENDING';
export const DELETE_BOOKING_SUCCESS = 'DELETE_BOOKING_SUCCESS';

export const SET_GUEST_PENDING = "SET_GUEST_PENDING";
